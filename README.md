...
then depend on `dev.get-milano:engine-compose:1.1.0`; the composite build substitutes it. On tagged releases the same coordinate is published to GitHub Packages Maven (note: GitHub Packages requires an authenticated Gradle repository even for public packages).
